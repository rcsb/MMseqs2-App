...
type SearchJob struct {
	...
	// Exported so that the request is a complete description of the job: with
	// per-instance job directories the worker that runs it has never seen the
	// job.fasta the server wrote, it only gets this struct out of redis.
